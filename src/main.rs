...
struct IndexBTreeLeafPage {
    raw_buf: Vec<u8>,
    header: BTreePageLeafHeader,
    cells: Vec<IndexBTreeLeafCell>,
}

struct IndexBTreeInteriorPage {
    raw_buf: Vec<u8>,
    header: BTreePageInteriorHeader,
    cells: Vec<IndexBTreeInteriorCell>,
}
...
impl IndexBTreeLeafPage {
    fn new(page: &[u8]) -> IndexBTreeLeafPage {
        let raw_buf = Vec::from(page);
        let header = BTreePageLeafHeader::new(&page[0..8]);
...
impl IndexBTreeInteriorPage {
    fn new(page: &[u8]) -> IndexBTreeInteriorPage {
        let raw_buf = Vec::from(page);
        let header = BTreePageInteriorHeader::new(&page[0..12]);
...
impl IndexBTreeLeafCell {
    fn new(page: &[u8], offset: usize) -> IndexBTreeLeafCell {
        let mut offset = offset;
        let mut payload_size: u64 = 0;
        offset += read_var_int(page, offset, &mut payload_size);
...
impl IndexBTreeInteriorCell {
    fn new(page: &[u8], offset: usize) -> IndexBTreeInteriorCell {
        let mut offset = offset;
        let left_child_pointer: u32 = read_u32(page,&mut offset);
